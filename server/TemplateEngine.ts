...
import { fileSystem } from "../misc/Globals";
        ...
        this.views.filter(n => !(/^[0-9]+$/.test(n)))
                  .forEach(n => {
                        ...
                        listener.get(`/${n}`.replace("index", ""), 
                                            (req: any, res: any)=> res.render(n, _interface.length > 0 ? _interface[0].prototype.getDataObject() : {}));
        ...
        this.views.filter(n => /^[0-9]+$/.test(n)).forEach(n =>
            {
                ...
                listener.use((req: any, res: any) => {
                    res.status(n);
                    res.render(n, _interface.length > 0 ? _interface[0].prototype.getDataObject() : {})
                });
            })
    }
