...
export class WebServer {
    ...
    public static set viewEngine(value: string) {
        this.listener.set("view engine", value);
    };

    /** Get the rootpath to the ejs views directory. */
    public static get ejsViewsRoot(): string {
        return this.listener.settings.views;
    }

    /** Get all absolute names of ejs files in ejs views directory. */
    public static get ejsViewNames(): string[] {
        return (Globals.fs.readdirSync(this.ejsViewsRoot) as string[])
            .filter(i => i.endsWith("ejs"))
            .map(i => i.substring(0, i.length - 4));
    }

    /** Get ejs view interfaces root */
    public static get viewInterfacesRoot(): string {
        return path.join(Globals.projectRoot, "src", "net", "views");
    }

    /** Get names of all ejs view interfaces */
    public static get viewInterfaceNames(): string[] {
        return (Globals.fs.readdirSync(this.viewInterfacesRoot) as string[]).filter(i => i.toLowerCase().endsWith(".ts")).map(i => i.substring(0, i.length - 3));
    }

    /** Get all viewnames and there prototype if available  */
    public static async getViewPrototypes(): Promise < Map < string, {
        [key: string]: IWebRequest
    } | undefined >> {
        let output: Map < string, {
                [key: string]: IWebRequest
            } | undefined > = new Map < string,
            {
                [key: string]: IWebRequest
            } | undefined > ();
        for (const ejsN of this.ejsViewNames) {
            ...
            if (interfaceName) {
                try {
                    ...
                    _interface = Object.assign({}, (modulePrototype as {
                        [key: string]: any
                    })[objectName]);
    ...
    private static setupListener({
        ...
        this.listener.use(express.urlencoded({
            extended: true
        }));
                ...
                if (/^[0-9]{3}$/.test(ejsName)) {
                    ...
                    if (callbackInterface) {
                        for (const requestType in callbackInterface) {
                            ...
                            switch (requestType) {
                                case "get":
                                    setter = () => this.listener.get(endpoint, renderViewCallback);
                                    break;
                                case "post":
                                    setter = () => this.listener.post(endpoint, renderViewCallback);
                                    break;
                                case "put":
                                    setter = () => this.listener.put(endpoint, renderViewCallback);
                                    break;
                                case "patch":
                                    setter = () => this.listener.patch(endpoint, renderViewCallback);
                                    break;
                                case "delete":
                                    setter = () => this.listener.delete(endpoint, renderViewCallback);
                                    break;
                                case "copy":
                                    setter = () => this.listener.copy(endpoint, renderViewCallback);
                                    break;
                                case "head":
                                    setter = () => this.listener.head(endpoint, renderViewCallback);
                                    break;
                                case "options":
                                    setter = () => this.listener.options(endpoint, renderViewCallback);
                                    break;
                                case "purge":
                                    setter = () => this.listener.purge(endpoint, renderViewCallback);
                                    break;
                                case "lock":
                                    setter = () => this.listener.lock(endpoint, renderViewCallback);
                                    break;
                                case "unlock":
                                    setter = () => this.listener.unlock(endpoint, renderViewCallback);
                                    break;
                                case "propfind":
                                    setter = () => this.listener.propfind(endpoint, renderViewCallback);
                                    break;
                            }
